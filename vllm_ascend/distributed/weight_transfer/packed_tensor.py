...
def packed_npu_ipc_producer(
    ...
    # ``reduce_tensor`` returns ``(rebuild_func, rebuild_args)``. Only the args
    # are stored; ``packed_npu_ipc_consumer`` rebuilds with the well-known
    # ``rebuild_npu_tensor`` (mirrors upstream vLLM's CUDA IPC engine).
...
def packed_npu_ipc_consumer(
    ...
    """
    # Imported lazily (matches upstream's inline import for the CUDA packed
    # consumer): ``rebuild_npu_tensor`` lives in ``torch_npu`` and must not be
